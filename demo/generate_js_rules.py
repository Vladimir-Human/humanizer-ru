...
scripts/check_demo_parity.py на общих векторах):
  ...
  - \\w -> [\\p{L}\\p{N}_], \\W — отрицание того же класса: Python \\w —
    буквенно-цифровые Unicode (категории L* и N*) плюс подчёркивание;
    комбинируемые знаки (M*) в Python \\w НЕ входят (re.match(r"\\w",
    "\\u0301") пуст), поэтому класс переноса не содержит \\p{M};
    JS \\w — только ASCII;
