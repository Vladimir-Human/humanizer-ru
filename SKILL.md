...
Пользователь сообщил дату создания текста?
   ...
   ↓
Прогнать regex по chatbot-artifacts.md (текст в файле и доступен Grep — готовыми подстроками из раздела «Детерминированная проверка»)
  ↓
...
Сумма от 45 до 50 — следы ИИ удалены. От 35 до 44 — приемлемо, есть что улучшить. Меньше 35 — переработать. Это самооценка агента перед сдачей правки; проверку без правки она не заменяет, а измеряемый контроль эффекта — за слепыми прогонами `eval/blind_eval.py`.
