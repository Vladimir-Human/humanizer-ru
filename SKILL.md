...
# Humanizer-ru — очеловечивание текста (v3.15.0)
...
Сосчитать мягкие признаки по категориям (содержательные, языковые, структурные, коммуникативные)
   Каждый признак считается один раз на текст; одно явление — один признак в одной категории (ось не добавляет признак, если то же явление уже учтено паттерном). Вердикт «написан ИИ» — только по жёстким основаниям Главного правила (маркер A / подлог источника / сведения автора); мягкие признаки вердикта не дают ни в каком сочетании, их счёт калибрует объём правки: 3–5 — выборочно, 6+ — целиком
   ↓
...
| `knowledge/corrections.md` | Журнал фидбека владельца (append-only): пользовательские правила правки, сильнее дефолтов справочников, но не отменяющие жёстких запретов | В начале аудита, если файл существует рядом |
