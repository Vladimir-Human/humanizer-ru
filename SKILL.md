...
- Исходный код, конфиги, технические логи — скилл только для связного текста. Файлы с метаданными (PNG/JPEG/GIF/WebP/SVG/PDF/DOCX/XLSX/PPTX/ODT/HTML/MD) скилл сам не правит: для них в репозитории лежит `scripts/filemarks/` — матрица применения в `references/removal-matrix.md`.
