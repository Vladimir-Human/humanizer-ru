...
- Текст не на русском языке. Отказаться и попросить русскоязычный текст.
...
правка запрошена → глубокая перезапись по rewrite-guide.md
   ↓
Если есть ссылки на источники → прогнать source-fabrication.md (без сети: офлайн-проверки дают флаги «требует проверки»)
   ↓
...
| `references/rewrite-guide.md` | Процедура выборочной правки: порядок по критичности, сохранение голоса и жанра, запрет на дописывание фактов, глубокая перезапись целиком машинного текста, петля самопроверки, два примера | Когда правка запрошена явно |
...
- ✓ Если есть ссылки на источники — все они прогнаны через офлайн-проверки `source-fabrication.md`, а спорные помечены как требующие проверки?
...
Сумма от 45 до 50 — следы ИИ удалены. От 35 до 44 — приемлемо, есть что улучшить. Меньше 35 — переработать. Это самооценка агента перед сдачей правки; проверку без правки она не заменяет, а измеримый контроль эффекта — за слепыми прогонами `eval/blind_eval.py`.
