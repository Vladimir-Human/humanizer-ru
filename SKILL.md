...
# Humanizer-ru — очеловечивание текста (v3.11.1)
...
Сосчитать мягкие признаки по категориям (содержательные, языковые, структурные, коммуникативные)
   Каждый признак считается один раз на текст. Вердикт «написан ИИ» допустим только по Главному правилу (маркер A / подлог источника / 3+ признаков из ≥2 категорий); счёт признаков калибрует объём правки: 3–5 — выборочно, 6+ — целиком
   ↓
...
**Проверка без правки.** Текст не изменяется. Ответ: вердикт одной строкой; список находок (цитата, паттерн, критичность); оговорка Главного правила, если вердикт опирается на мягкие признаки; предложение переписать — только если пользователь попросит. При 0–2 признаках или признаках только одной категории вердикт не выносится — ответ: «оснований для вердикта нет» + находки.
