...
авторстве нет. Для английского и смешанного текста доступны профили
`--language en` и `--language auto` в командах очистки и сверки фактов;
русские стилевые эвристики к английскому не применяются. Исходный код
не обрабатывается; в юридических документах и договорах применяется только
...
```sh
git clone --branch v3.36.1 --depth 1 https://github.com/Vladimir-Human/humanizer-ru.git ~/.claude/skills/humanizer-ru
```
