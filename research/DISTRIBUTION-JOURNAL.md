...
- 2026-09-11 (вечер): внешние авторские каналы — публикации выполняет
  контур сопровождения; блокировка «публикация только владельцем» снята
  11.09.2026 решением держателя аккаунтов, авторизации площадок
  предоставлены держателем. Habr: первая внешняя авторская статья «Два
  humanizer-ru: я отозвал свою лучшую метрику и зову тёзку на бенчмарк»
  ...
  (прямые кавычки — типографика зафиксирована решением держателя), check_outward
  ...
  дубликат удалён в тот же вечер, единственный листинг —
  ...
  площадки в сессии); Telegram: запуск отложен.
...
- 2026-09-12: переписывание истории репозитория по решению держателя
  аккаунта — из метаданных коммитов (автор/коммиттер) удалены личные
  почтовые адреса, заменены на эквиваленты users.noreply.github.com.
  Имена авторов и содержимое файлов не менялись: деревья всех коммитов
  байт-идентичны предотменённой истории, число коммитов в main сохранено
  (962). Все ветки и теги перевыпущены на новых sha (force-push);
  полный список тегов (77) пересоздан на новых коммитах, изначально
  подписанные переподписаны тем же GPG-ключом проекта 023E1F146B59348F,
  сообщения и даты теггера сохранены. Резервный бандл с оригинальной
  историей хранится у держателя аккаунта. Старые sha коммитов,
  цитируемые в этом журнале, указывают на предотменённую историю:
  до очистки кэшей GitHub они остаются разрешимы через refs/pull/*
