...
Новый файл в v2.3. Справочник разбит на части: эталонные пары «образец / результат» для всех регулярных выражений из `references/chatbot-artifacts-links.md` и `references/chatbot-artifacts-markup.md` собраны в файлах из таблицы ниже. Эти образцы используются для проверки регулярных выражений перед публикацией изменений в скилле.
...
При смене флагманской модели (см. `references/llm-fingerprints.md`) нужно:
