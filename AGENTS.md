...
```sh
python scripts/check_all.py        # 85 гейтов (81 в --quick)
python -m unittest discover -s tests
```

Полный прогон идёт около 4 минут (самый долгий гейт — целостность blind-eval,
~2 минуты). Таймаут вызова меньше 5 минут обрывает зелёный прогон и выглядит как
падение гейтов: при автоматическом запуске закладывай таймаут >= 5 минут или
используй `--quick` (81 гейт) для быстрой проверки.
