...
```sh
python scripts/check_all.py        # 58 гейтов (54 в --quick)
...
## Исходящие тексты (issues, PR, сторонние сервисы)

Публичное утверждение существует только в той форме, которую проверяет
команда, упавшая бы на ложном. Перед отправкой черновика:

- python3 scripts/check_outward.py черновик.md — ловит ESC/mojibake,
  локальные пути, внутренние роли и подсвечивает притяжательные претензии
