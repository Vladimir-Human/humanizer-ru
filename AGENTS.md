...
```sh
python scripts/check_all.py        # 150 гейтов (139 в --quick)
...
(139 гейтов, ~2 минуты) для быстрой проверки.
...
- Теги и Releases: выполняются автономным контуром сопровождения по
  ...
  существующим GPG-ключом проекта, публикация через CI, не менее 24 часов
  с предыдущей публикации (`scripts/check_release.py
  --pre-release-interval`); опубликованные теги не пересоздавать, ключ не
  подменять, секреты не выводить.
