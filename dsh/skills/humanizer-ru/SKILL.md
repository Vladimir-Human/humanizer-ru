...
compatibility: DeepSeek Harness (dsh), Claude.ai, Claude Code, opencode и другие агенты с поддержкой agentskills.io. Только текст, без выполнения кода и сети.
metadata:
...
# Humanizer-ru — очеловечивание текста (v3.17.0)
...
| `references/chatbot-artifacts.md` | Маркеры классов A и B с регулярными выражениями: следы OpenAI, Grok, Gemini, Perplexity, DeepSeek, невидимые символы. Части: links, markup, legacy | При подозрении на копирование из чата |
...
| `references/source-fabrication.md` | Проверка ссылок: офлайн (формат DOI, книга без страниц, неиспользуемая сноска) и с разрешения пользователя (404, чужая статья, несуществующий ISBN) | Всегда, если есть ссылки на источники |
...
| `references/false-positives.md` | Что не считается признаком ИИ: длинное тире в художке, автозамена кавычек, правило трёх, канцелярит в юридическом тексте; разбор Главного правила | Перед вердиктом о машинном происхождении |
| `references/removal-matrix.md` | Матрица удаления: какой слой (A/B/файлы) снимает какую метку поставщика, что вне покрытия | Просьба снять метки с файла |
...
| `tests/test-fixtures.md` | Части: `tests/test-fixtures-cases.md` (образцы 1–15), `tests/test-fixtures-pairs.md` (пары «до / после»). Проверочные данные, в бандл скилла не входят | При обновлении скилла |
| `knowledge/corrections.md` | Журнал обратной связи владельца (append-only): пользовательские правила правки, сильнее дефолтов справочников | В начале аудита, если файл существует рядом |

Валидаторы и прогоны в `scripts/` и `eval/` — инструменты разработчика: агент их не запускает, они описаны в README. Разделы справочников `references/*` идут по общей схеме: проблема, маркеры, граница ложного срабатывания, пример — повторяющаяся форма есть навигационное удобство, а не сигнал генерации; о симметрии в авторских текстах — паттерн #13 в `references/language-patterns.md`.
...
- [x] Ссылки прогнаны через офлайн-проверки `references/source-fabrication.md`, спорные помечены «требует проверки»?
...
- [x] «является / представляет собой» заменены на «это» (кроме академического регистра, false-positives.md §11)?
- [x] Тройки правила трёх заменены на двойки (на четвёрки — только при готовом четвёртом элементе)?
...
Сумма пяти критериев без «Фактов», максимум 50: 45–50 — следы ИИ удалены;
35–44 — приемлемо; ниже 35 —
переработать. «Факты» ниже 8 — откат правки целиком: дописанные факты
стилем не оправдать (`references/rewrite-guide.md`, «Проверка после
правки»). Самооценка агента; измеримый контроль — `eval/blind_eval.py`.
