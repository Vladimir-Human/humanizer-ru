...
Извлечение БЕЗ ML, только стандартная библиотека; словарная морфология
(pymorphy3) — optional extra humanizer-ru[ru-morph]: без неё сравнение имён
идёт регистр-независимо, с ней — по лемме (поправка AMENDMENT-F10-F15,
2026-09-04).

Категории фактов:
  numbers   — цифры и числительные словами (таблица 0-999 + разряды),
              канон значения = int + каноническая единица, поэтому
              «15 %» и «пятнадцать процентов» — один факт;
  dates     — дд.мм.гггг, дд месяц гггг, месяц гггг, гггг год/г., ISO;
  urls      — http(s)-ссылки и www.;
  emails    — адреса почты;
  names     — последовательности из 2+ заглавных слов (лемма при наличии
              ru-morph, иначе регистр-независимо);
...
# optional extra ru-morph: словарная морфология, не ML
...
_SEP = " \u00a0\u202f"
NUM_RX = re.compile(r"\d+(?:[%s]\d{3})*(?:[.,]\d+)?" % _SEP)
...
UNIT_RX = re.compile(
    r"\s?(?:%s)" % "|".join(sorted((re.escape(w) for w in UNIT_CANON),
...
def extract(text: str) -> Dict[str, List[dict]]:
    ...
    for m in NUM_RX.finditer(text):
        ...
        out["numbers"].append({"value": value, "raw": m.group(),
                               "pos": m.start()})
