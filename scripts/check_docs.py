...
TOP_LEVEL_MANIFEST = frozenset((
 ...
 "glama.json",
 # 2026-09-11: граница контекста Codex-агента (.codexignore: исследовательские
 # корпусы, фикстуры, локальное состояние — не поставка скилла) и снимок
 # разрешённых зависимостей uv.lock (runtime-зависимостей нет; фиксирует
 # состав для воспроизводимой сборки).
 ".codexignore", "uv.lock",
))
...
def check_repo(root):
 ...
 # 2026-09-07: потолок SKILL осознанно поднят 15750 -> 15850 — приказ
 # цикла требует убрать обещание «естественность без искажения смысла»
 # и уточнить датированную ветку; явная формулировка границ длиннее
 # запрещённого обещания, сокращение прочих разделов не компенсирует.
 # 2026-09-07 (вторая правка того же цикла): потолок 15850 -> 16050 —
 # «Когда не применять» приведено в согласие с деревом решений
 # (юридические документы: очистка артефактов класса A применима) и
 # датированная ветка сформулирована честно (подтверждённая дата
 # исключает чат-артефакты LLM-эпохи, но не машинный текст не-LLM
 # инструментов); обе формулировки не становятся короче прежних без
 # потери явности границ.
 for _rel18, _unit18, _limit18 in (
   ("SKILL.md", "chars", 16050),
   ("README.md", "lines", 320),
