#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""check_outward.py — валидатор черновика исходящего текста (issue, PR, сервис).

Файлы репозитория проверяет общий чек-лист, а публичный комментарий
уходит получателю без единой проверки. Этот валидатор закрывает ровно те
провалы, которые уже случались: нечитаемые управляющие последовательности,
утечки локальных путей и внутренних ролей, и — предупреждением — категоричные
утверждения об отсутствующем или о прошлом («не существует в репозитории»,
«история переписана», «раньше было только»), каждое из которых обязано
быть подкреплено командой, упавшей бы на ложном: «не вижу» и «нет» —
разные утверждения.
...
Коды возврата: 0 — чисто (WARN не мешает); 1 — есть FAIL (в том числе
нечитаемый файл); 2 — не прочитан ни один входной файл. Только
...
CLAIM_RX = [
    re.compile(r"(?:не существует|does not exist|doesn't exist|no longer exists)"
               r"[^.]{0,80}(?:репозитор|repos)", re.I),
...
def check_files(paths):
    ...
    for p in paths:
        try:
            ...
            print("[FAIL] %s: не читается: %s" % (p, exc))
            fails_total += 1
            continue
        readable += 1
...
def selftest():
    ...
    case("локальный путь -> FAIL", "проверка в C:\\Users\\test\\repo\\file.md",
         want_fail=True)
    case("unix-путь -> FAIL", "взял из /Users/john/repo/file.md",
         want_fail=True)
    case("домашний путь ~ -> FAIL", "патч лежит в ~/home/user/x.diff",
         want_fail=True)
