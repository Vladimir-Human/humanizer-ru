...
("benchmark: самопроверка", [PY,
        ...
        ("action-yaml: самопроверка", [PY, "scripts/check_action_yaml.py",
                                       "--selftest"], [], {0}),
        ("action-yaml: экшен и workflows парсятся",
         [PY, "scripts/check_action_yaml.py"],
         ["action/action.yml"], {0}),
    ...
    gates += [
        ...
        ("identity: identity.v1.json синхронен с носителями",
         ...
         ["scripts/check_identity.py", "identity.v1.json", "llms.txt",
          "pyproject.toml"], {0}),
        ("install-receipt: самопроверка",
         [PY, "scripts/install_receipt.py", "--selftest"],
         ["scripts/install_receipt.py"], {0}),
