...
Регулярные выражения для следов копирования из чат-ботов. Один такой маркер в обычном тексте — почти точно ИИ. Все выражения прогоняются автоматически: `python3 scripts/check_markers.py` — три уровня образцов на каждое, прогон обязателен в CI (23 из 23 проходят). Тот же скрипт проверяет произвольный текст: `python3 scripts/check_markers.py --scan файл.md`. Третий workflow (`self-scan.yml`) прогоняет эти же выражения по собственному тексту проекта при каждом изменении.
