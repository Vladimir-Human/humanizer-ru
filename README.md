...
|---|---|---|
| [Vladimir-Human/humanizer-ru](https://github.com/Vladimir-Human/humanizer-ru) — этот проект | Редактор русского текста: 38 паттернов, 38 regex-маркеров с реестром доказательств 36/38, 33 CI-гейта, слепые парные прогоны | Детекторы не обходим и текст под них не подгоняем; цель — естественный текст, а не зелёный вердикт детектора |
...
Полная запись доказательств — неизменяемая ссылка на источник, дата обращения, дословный образец, класс доказательства и fixture — есть у 36 из 38 маркеров. Остальные закрыты только фикстурами. Валидатор печатает это число как есть, а не округляет его до удобного.
