...
Версия action закрепляется не входом `ref`, а строкой `uses: Vladimir-Human/humanizer-ru/action@v3.16.9`.
...
Доли «0 из N» публикуются с интервалом и с датой. Снимок прогона 2026-08-25: ноль ложных срабатываний на 11 человеческих текстах, 0/11, Wilson 95% CI [0%; 25.9%]. Это наблюдение, а не свойство детектора. В реплике той же линии судьи от 2026-08-31 два текста из 11 превысили порог: 2/11, CI [5.1%; 47.7%]. Без даты ни одна величина не цитируется (ERRATA.md). Интервалы пересчитывает scripts/check_confidence.py.
...
│   ├── check_action_yaml.py     # action.yml парсится как YAML
...
Полный чек-лист — одна команда: `python scripts/check_all.py` — 75 гейтов полного чек-листа (71 в --quick). Юнит-тесты — `python -m unittest discover -s tests`.
