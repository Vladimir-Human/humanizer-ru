...
Регулярные выражения для следов копирования из чат-ботов. Один такой маркер в обычном тексте — почти точно ИИ. Все выражения прогоняются автоматически: `python3 scripts/check_markers.py` — три уровня образцов на каждое, прогон обязателен в CI (34 из 34 проходят). Тот же скрипт проверяет произвольный текст: `python3 scripts/check_markers.py --scan файл.md`. Третий workflow (`self-scan.yml`) прогоняет эти же выражения по собственному тексту проекта при каждом изменении.
...
| `[cite: 8]`, `[Cite: 12]` | Google Gemini (ссылка на фрагмент источника) | `\[[Cc]ite:\s?\d+\]` |
